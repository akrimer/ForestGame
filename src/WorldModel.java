...
public final class WorldModel {
    ...
    public static void parseDude(WorldModel world, String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Dude.DUDE_NUM_PROPERTIES) {
            Dude_Not_Full entity = Dude_Not_Full.createDudeNotFull(id, pt, Double.parseDouble(properties[Dude.DUDE_ACTION_PERIOD]), Double.parseDouble(properties[Dude.DUDE_ANIMATION_PERIOD]), Integer.parseInt(properties[Dude.DUDE_LIMIT]), imageStore.getImageList(Dude.DUDE_KEY));
            world.tryAddEntity(entity);
        }else{
    ...
    public static void parseHouse(WorldModel world, String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == House.HOUSE_NUM_PROPERTIES) {
            House entity = House.MakeHouse(id, pt, imageStore.getImageList(House.HOUSE_KEY)); // I am not getting an ID for TrampleFull
            world.tryAddEntity(entity);
        }else{
